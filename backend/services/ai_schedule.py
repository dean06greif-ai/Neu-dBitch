...
def next_aligned_ts(now_ts: float, interval_min: int, tz) -> float:
    """Nächster Analyse-Zeitpunkt am Voll-Stunden-Raster (lokale Zeitzone `tz`).

    Läufe fallen immer auf 'gerade' Uhrzeiten: bei 15 min -> :00/:15/:30/:45,
    bei 30 min -> :00/:30 usw. Anker ist der Beginn der aktuellen Stunde."""
    from datetime import datetime as _dt
    interval_s = max(1, int(interval_min)) * 60
    dt = _dt.fromtimestamp(now_ts, tz)
    anchor = dt.replace(minute=0, second=0, microsecond=0).timestamp()
    n = int((now_ts - anchor) // interval_s) + 1
    return anchor + n * interval_s
