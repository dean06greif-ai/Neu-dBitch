...
def test_ki_trader_label(H):
    ...
    import subprocess
    subprocess.run(["python3","-c","import os; from pymongo import MongoClient; db=MongoClient(os.environ.get('MONGO_URL','mongodb://localhost:27017'))[os.environ.get('DB_NAME','crypto_scanner')]; db.settings.delete_one({'_id':'trade_guard_state'}); db.auto_trades.delete_many({'mode':'paper'})"], check=False)
    ...
    for side in ["LONG","SHORT"]:
        ...
        if open_ones:
            t = open_ones[0]
            # Website-Trades (source=manuell) heißen bewusst 'Manuell (Website)';
            # wichtig ist nur: kein '(Custom)'-Suffix und ein sauberes Label.
            assert t.get("strategy_name") in ("KI Trader", "Manuell (Website)"), \
                f"unerwartetes Label: {t.get('strategy_name')!r}"
