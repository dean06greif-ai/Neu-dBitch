...
def paper_trade(api, auth):
    ...
    # Alt-Trades entfernen, damit der Anti-Stacking-Cooldown nicht blockt.
    try:
        from pymongo import MongoClient
        _db = MongoClient(os.environ.get("MONGO_URL", "mongodb://localhost:27017"))[
            os.environ.get("DB_NAME", "crypto_scanner")]
        _db.auto_trades.delete_many({"mode": "paper", "symbol": "BTCUSDT"})
        _db.settings.update_one({"_id": "trade_guard_state"},
    ...
    mine = [t for t in lst
            if t.get("ai_reasoning") == "test_iter_chat_cmds_e2e" and t.get("symbol") == "BTCUSDT"
...
def test_strategy_assist_without_key(api, auth):
    ...
    if r.status_code == 200:
        # Umgebung MIT LLM-Key: sauberes Feedback statt Fehler
        assert (r.json() or {}).get("status") == "ok", r.text[:200]
        return
