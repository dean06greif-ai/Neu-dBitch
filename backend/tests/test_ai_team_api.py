...
Runs against the public preview URL (REACT_APP_BACKEND_URL) and validates:
  - /api/ai/status shape (roles=6, provider_keys incl. cerebras, backup_keys, ...)
...
def test_post_roles_persists_analyst(admin_headers):
    ...
    assert an.get("fallback_model") == "openai/gpt-oss-20b"
...
def test_deep_analyze_graceful_rate_limit(admin_headers):
    # Mit echten Keys läuft eine komplette Tiefenanalyse (LLM) – braucht Zeit.
    r = requests.post(f"{BASE_URL}/api/ai/deep-analyze", headers=admin_headers, timeout=240)
