...
def _gate_samples() -> int:
    try:
        r = requests.get(f"{BASE_URL}/api/ml/gate/dataset", timeout=60)
        return int(((r.json() or {}).get("dataset") or {}).get("samples") or 0)
    except Exception:
        return 0


# Diese Regressionstests prüfen das trainierte Gate auf der Produktions-DB
# (>=900 Samples, prod_readonly). In Umgebungen ohne diese Daten: skip.
if _gate_samples() < 120:
    pytest.skip("Zu wenig ML-Gate-Daten in dieser Umgebung (Prod-Regressionstests)",
