...
def test_auto_switch_uses_role_fallback_and_rollback_restores():
    ...
    assert switches[0]["to"]["model"] == "openai/gpt-oss-120b"
    assert role_manager.config["analyst"]["model"] == "openai/gpt-oss-120b"
...
def test_auto_switch_also_on_explicit_recommendation():
    ...
    roles = [{"role": "summarizer", "verdict": "inaktiv", "action": "modell_wechseln",
              ...
              "suggested_provider": "groq", "suggested_model": "openai/gpt-oss-120b"}]
    switches = asyncio.run(s._auto_switch(roles))
    assert switches[0]["to"]["model"] == "openai/gpt-oss-120b"
    _reset_roles()


def test_auto_switch_cascades_to_fallback_when_already_active():
    """Vorschlag = aktives Modell -> Kaskade nimmt Fallback 1 der Rolle."""
    _reset_roles()
    s = _sup()
    s.settings["auto_switch"] = True
    ...
    assert len(switches) == 1
    assert switches[0]["to"]["model"] != "openai/gpt-oss-120b"
    assert "Fallback" in (switches[0]["to"].get("via") or "")
    _reset_roles()
