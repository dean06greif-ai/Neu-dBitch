...
def test_ml_gate_status():
    ...
    if not j.get("model_loaded") and "Zu wenig Daten" in str(j.get("last_error") or ""):
        pytest.skip("Zu wenig Trainingsdaten in dieser Umgebung – Gate-Status ok")
...
def _make_dc_trade(mongo_coll, dc=True, symbol="QAFAKEXYZUSDT", entry=100.0, sl=99.0):
...
def test_guard2_sl_ratchet_blocked_too_close(mongo_coll, auth_headers):
    ...
    tid = _make_dc_trade(mongo_coll, dc=False, symbol="QAFAKEXYZUSDT",
                         entry=100.0, sl=99.0)
    try:
