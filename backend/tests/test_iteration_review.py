...
def qa_strategies(auth_headers):
    ...
    # Unbekannter Indikator wird seit der Regel-Validierung direkt beim
    # Anlegen abgewiesen (422 + Korrektur-Vorschläge) statt erst im Backtest.
...
def test_backtest_strategy_warnings_unknown_indicator(qa_strategies, auth_headers):
    """Unbekannter Indikator: Anlegen wird mit 422 + Fix-Vorschlag abgewiesen
    (früher: Warnung erst im Backtest)."""
    r = requests.post(f"{BASE_URL}/api/strategies/custom", headers=auth_headers,
                      json=QA_BAD, timeout=TIMEOUT)
    assert r.status_code == 422, r.text
    detail = (r.json() or {}).get("detail") or {}
    text = str(detail).lower()
    assert "supertrend" in text, f"Problem-Meldung fehlt: {detail}"
    assert detail.get("fixes"), f"Korrektur-Vorschläge fehlen: {detail}"
