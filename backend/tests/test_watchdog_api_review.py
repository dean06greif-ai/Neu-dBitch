...
class TestWatchdog:
    def test_status_returns_configured_false_and_settings(self):
        ...
        assert r.status_code == 200
        data = r.json()
        # configured hängt davon ab, ob Bitunix-Keys in der Umgebung stecken
        assert data.get("configured") in (True, False)
        s = data.get("settings")
    ...
    def test_run_with_admin_returns_skipped(self, auth_headers):
        ...
        data = r.json()
        # Ohne Bitunix-Keys: skipped. Mit Keys läuft der Lauf durch: success.
        assert data.get("status") in ("skipped", "success"), data
